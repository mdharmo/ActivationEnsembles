...
import lasagne
import urllib.request as urllib2  # For downloading the sample text file. You won't need this if you are providing your own file.
from ZDense import Zupdate
from ZGru import ZGRULayer
import activations as act
from ZGru import ZGate

def build_zgru(vocab_size,input_var = None,nonlins=lasagne.nonlinearities.rectify):
    ...
    l_forward_1 = ZGRULayer(
        l_in, 200,
        resetgate=ZGate(W_cell=None,nonlinearity=nonlins),
        updategate=ZGate(W_cell=None,nonlinearity=nonlins),
        hidden_update=ZGate(W_cell=None,nonlinearity=nonlins))
    acts = [len(l_forward_1.nonlinearity_hid)]*3
    my_units = [200]*3

    l_forward_2 = ZGRULayer(
        l_forward_1, 200,
        resetgate=ZGate(W_cell=None, nonlinearity=nonlins),
        updategate=ZGate(W_cell=None, nonlinearity=nonlins),
        hidden_update=ZGate(W_cell=None, nonlinearity=nonlins),
    ...
    acts += [len(l_forward_2.nonlinearity_hid)]*3
    my_units += [200]*3
    ...
    out = lasagne.layers.DenseLayer(l_forward_2, num_units=vocab_size, W=lasagne.init.Normal(),
                                      nonlinearity=lasagne.nonlinearities.softmax)
    return   out,act,my_units
...
def main(dataset,num_epochs,parname,step,bn,actchoice,zstep):
    ...
    nonlins = [T.nnet.sigmoid, act.n_tanh,act.n_relu,act.n_invabs,act.n_softrelu]


    network,act,my_units = build_zgru(vocab_size,input_var,nonlins)
    ...
    #updates = Zupdate(cost, all_params, LEARNING_RATE)
    updates = Zupdate(cost, all_params, acts, my_units, learning_rate=LEARNING_RATE, zstep=LEARNING_RATE)
