...
from MarkObjective import binary_crossentropy
from lasagne.layers import Conv2DLayer, MaxPool2DLayer,LocalResponseNormalization2DLayer,InputLayer,SliceLayer,concat
from lasagne.layers import Upscale2DLayer,TransposedConv2DLayer
...
def build_cnn(input_var=None):
    ...
    e1 = lasagne.layers.Conv2DLayer(l_in, num_filters=32, filter_size=5,pad='same',nonlinearity=lasagne.nonlinearities.rectify, W=lasagne.init.HeNormal(gain='relu'))
    e2 = lasagne.layers.MaxPool2DLayer(e1,pool_size=2,stride=2)
    e3 = lasagne.layers.Conv2DLayer(e2,num_filters=64,filter_size=5,pad='same',nonlinearity=lasagne.nonlinearities.rectify, W=lasagne.init.HeNormal(gain='relu'))
    e4 = lasagne.layers.MaxPool2DLayer(e3,pool_size=2,stride=2)
    e5 = lasagne.layers.Conv2DLayer(e4,num_filters=128,filter_size=5,pad='same',nonlinearity=lasagne.nonlinearities.rectify, W=lasagne.init.HeNormal(gain='relu'))
    e6 = lasagne.layers.MaxPool2DLayer(e5,pool_size=6)

    d6 = lasagne.layers.Upscale2DLayer(e6,scale_factor=6)
    d5 = lasagne.layers.TransposedConv2DLayer(d6, e5.input_shape[1],e5.filter_size, stride=e5.stride, crop=e5.pad,W=e5.W, flip_filters=not e5.flip_filters,b=None)
    d4 = lasagne.layers.Upscale2DLayer(d5,scale_factor=2)
    d3 = lasagne.layers.TransposedConv2DLayer(d4, e3.input_shape[1],e3.filter_size, stride=e3.stride, crop=e3.pad,W=e3.W, flip_filters=not e3.flip_filters,b=None)
    d2 = lasagne.layers.Upscale2DLayer(d3, scale_factor=2)
    d1 = lasagne.layers.TransposedConv2DLayer(d2, e1.input_shape[1], e1.filter_size, stride=e1.stride, crop=e1.pad, W=e1.W,flip_filters=not e1.flip_filters,nonlinearity=lasagne.nonlinearities.sigmoid,b=None)
...
def main(set1,set2,set3,num_epochs,parname,storesave):
    # Load the dataset
    ...
    #updates = lasagne.updates.momentum(loss,params,learning_rate=0.001,momentum=0.9)
