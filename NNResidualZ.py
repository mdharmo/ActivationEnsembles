...
def load_data():
    xs = []
    ys = []
    d = unpickle('/home/data/cifar-100-python/train')
    x = d['data']
    y = d['fine_labels']
    xs.append(x)
    ys.append(y)

    d = unpickle('/home/data/cifar-100-python/test')
    xs.append(d['data'])
...
from MarkObjective import categorical_crossentropy as mark_entropy

def build_cnn(nonlins,input_var=None, n=5):
    # create a residual learning building block with two stacked 3x3 convlayers as in paper
    def residual_block(l,my_units,acts, increase_dim=False, projection=False):
        input_num_filters = my_units[-1]
        if increase_dim:
            first_stride = (2, 2)
        ...
        stack_1 = ConvLayer(l, num_filters=out_num_filters, filter_size=(3, 3), stride=first_stride, nonlinearity=None,
                      pad='same', W=lasagne.init.HeNormal(gain='relu'), flip_filters=False)

        my_units +=[out_num_filters]
        acts += [len(nonlins)]
        stack_1 = MaxMinNorm(stack_1,num_units=out_num_filters,nonlinearity=nonlins)
    ...
    for _ in range(n):
        l,my_units,acts = residual_block(l,my_units,acts)

    # second stack of residual blocks, output is 64 x 16 x 16
    l,my_units,acts = residual_block(l,my_units,acts, increase_dim=True)
    for _ in range(1, n):
        l,my_units,acts = residual_block(l,my_units,acts)
...
def main(n=5, num_epochs=82, model=None,actchoice = 0):
    # Check if cifar data exists
    if not os.path.exists("/home/data/cifar-100-python"):
        print(
            "CIFAR-100 dataset can not be found. Please download the dataset from 'https://www.cs.toronto.edu/~kriz/cifar.html'.")
        return
    ...
    network,my_units,acts = build_cnn(nonlins,input_var, n)
    ...
    if model is None:
        ...
        loss = mark_entropy(prediction, target_var)
        loss = loss.mean()
        # add weight decay
        ...
        updates = Zupdate(loss,params,acts,my_units,learning_rate=sh_lr,momentum=0.9)
        print('Finish update')
        # Compile a function performing a training step on a mini-batch (by giving
        # the updates dictionary) and returning the corresponding training loss:
        train_fn = theano.function([input_var, target_var], loss, updates=updates)
        print('Finish Train Function')
    # Create a loss expression for validation/testing

    test_prediction = lasagne.layers.get_output(network, deterministic=True)
    test_loss = mark_entropy(test_prediction,
                                                            target_var)
    ...
    if model is None:
        ...
        for epoch in range(num_epochs):
            ...
            for batch in iterate_minibatches(X_train, Y_train, 128, shuffle=True, augment=True):
                ...
                train_batches += 1
                #print(np.max(cz.delta.get_value()[:, :].tolist()))
                #print(np.max(cz.eta.get_value()[:, :].tolist()))
    ...
    return test_err/test_batches, test_acc/test_batches * 100
'''
if __name__ == '__main__':
    if ('--help' in sys.argv) or ('-h' in sys.argv):
        print("Trains a Deep Residual Learning network on cifar-100 using Lasagne.")
        print(
