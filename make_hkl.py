# Preprocessing: From JPEG to pkl
...
def get_img(img_name, img_size=224, batch_size=256):
...
def save_batches(file_list, tar_dir, img_size=224, batch_size=256,
    ...
    for file_name in file_list:
        ...
        if count % batch_size == 0:
            ...
            if num_sub_batch == 1:
                save_name = '%04d' % (batch_count - 1) + '.pkl'
                percentage_done = (float((batch_size*(batch_count)))/len(file_list))*100
                print(percentage_done)
                pkl.dump(img_batch, open(os.path.join(tar_dir, save_name), mode='wb'))
            elif num_sub_batch == 2:
                half_size = batch_size / 2
                save_name = '%04d' % (batch_count - 1) + '_0.pkl'
                pkl.dump(img_batch[:, :, :, :half_size],
                         open(os.path.join(tar_dir, save_name), mode='wb'))

                save_name = '%04d' % (batch_count - 1) + '_1.pkl'
                pkl.dump(img_batch[:, :, :, half_size:],
                         open(os.path.join(tar_dir, save_name), mode='wb'))
                ...
                save_name = '%04d' % (batch_count - 1) + '_00.pkl'
                pkl.dump(img_batch[:, :, :, :q1],
                         open(os.path.join(tar_dir, save_name), mode='wb'))
                save_name = '%04d' % (batch_count - 1) + '_10.pkl'
                pkl.dump(img_batch[:, :, :, q1:q2],
                         open(os.path.join(tar_dir, save_name), mode='wb'))
                save_name = '%04d' % (batch_count - 1) + '_01.pkl'
                pkl.dump(img_batch[:, :, :, q2:q3],
                         open(os.path.join(tar_dir, save_name), mode='wb'))
                save_name = '%04d' % (batch_count - 1) + '_11.pkl'
                pkl.dump(img_batch[:, :, :, q3:],
                         open(os.path.join(tar_dir, save_name), mode='wb'))
            else:
...
def get_train_filenames(src_train_dir, misc_dir, seed=1):
    ...
    np.save(os.path.join(misc_dir, 'shuffled_train_filenames.npy'),train_filenames)
...
if __name__ == '__main__':
    ...
    if gen_type == 'full':
        ...
        NotImplementedError("gen_type (2nd argument of make_pkl.py) can only be full or toy")
