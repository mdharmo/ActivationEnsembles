...
def div_labels(src_file, orig_batch_size, num_div):
    ...
    if num_div == 2:
        ...
        for ind in range(labels.size / batch_size):
            ...
            assert np.all(labels.reshape((-1, batch_size))[ind] == labels[batch_size * ind: batch_size * (ind + 1)])

            labels_sub = labels_1 if ind % 2 else labels_0

            ind_sub = ind / 2
            assert np.all(labels[batch_size * ind: batch_size * (ind + 1)] ==labels_sub[batch_size * ind_sub: batch_size * (ind_sub + 1)])
