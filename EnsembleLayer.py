...
class EnsembleLayer(lasagne.layers.Layer):
    """
    ...
    **kwargs
    ...
    def get_output_for(self, input, deterministic=False,
        ...
        for k in range(len(self.nonlinearity)):

            act = self.nonlinearity[k](input)
            input_mean = T.set_subtensor(input_mean[k],T.min(act,axis=self.axes))
            input_inv_std = T.set_subtensor(input_inv_std[k],(T.max(act,axis=self.axes)-input_mean[k] + self.epsilon))
            ...
            if update_averages and k == (len(self.nonlinearity)-1):
            ...
            tempz = self.z[k].dimshuffle(pattern)
            tempdelta = self.delta[k].dimshuffle(pattern)
            tempmean = mean[k].dimshuffle(pattern)
            tempstd = inv_std[k].dimshuffle(pattern)
            tempeta = self.eta[k].dimshuffle(pattern)
            # normalize
            normalized = tempz*(tempdelta*((act - tempmean)/ tempstd)+tempeta)
            val+= normalized
