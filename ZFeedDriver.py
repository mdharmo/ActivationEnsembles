...
for a in range(1):
    actchoice = a+1
    actsave = '/Act_' + str(actchoice)
    bn = True
    bndir = '/BN'
    storeacc = np.zeros(numtests)
    storeloss = np.zeros(numtests)
    for t in range(numtests):
        ...
        step = 1.0
        zstep = 0.005
        save = '/home/mharmon/ZProject/ModelResults/mnist/FeedFinalAda' + bndir + actsave
        ...
        bestperc,finallayer1zstore,finallayer2zstore,finallayer3zstore = main(dataset,num_epochs,parname,step,bn,actchoice,zstep,seed)
        ...
        newlabels = ['Neuron 1', 'Neuron 2', 'Neuron 3', 'Neuron 4', 'Neuron 5']
        plt.figure(pngcount,figsize=(8,6))
        xvec = np.linspace(-1,1,num=20)
        for j in range(5):
            plt.plot(xvec,val1[j, :], linewidth=2.0, color=mycolors[j], label=newlabels[j])
            plt.ylabel('Function Value')
            plt.xlabel('X')
        savestring = figsave + 'FunksCompareLayer1' + '.png'
        titlestring = 'Function Set ' +str(a+1)
        plt.title(titlestring)
        ...
        plt.figure(pngcount,figsize=(8,6))
        for j in range(5):
            plt.plot(xvec,val2[j, :], linewidth=2.0, color=mycolors[j], label=newlabels[j])
            plt.ylabel('Function Value')
            plt.xlabel('X')
        ...
        plt.figure(pngcount,figsize=(8,6))
        for j in range(5):
            plt.plot(xvec,val3[j, :], linewidth=2.0, color=mycolors[j], label=newlabels[j])
            plt.ylabel('Function Value')
            plt.xlabel('X')
