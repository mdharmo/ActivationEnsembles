...
class ZGate(object):
    ...
    def __init__(self, W_in=init.Normal(0.1), W_hid=init.Normal(0.1),
                 W_cell=init.Normal(0.1), b=init.Constant(0.), z = init.Constant(1.0),
        ...
        if W_cell is not None:
            self.W_cell = W_cell
        self.b = b
        z = init.Constant(1.0 / float(len(nonlinearity)))
        self.z = z
...
class ZGRULayer(MergeLayer):
    ...
    def __init__(self, incoming, num_units,
                 resetgate=ZGate(W_cell=None),
                 updategate=ZGate(W_cell=None),
                 hidden_update=ZGate(W_cell=None,nonlinearity = [T.tanh]),
        ...
        super(ZGRULayer, self).__init__(incomings, **kwargs)
        ...
        def add_gate_params(gate, gate_name):
            ...
            return (self.add_param(gate.W_in, (num_inputs, num_units),
                    ...
                    self.add_param(gate.b, (num_units,),
                                   ...
                                   regularizable=False),
                    self.add_param(gate.z, (len(gate.nonlinearity),num_units),
                                   name="z_{}".format(gate_name),
                                   regularizable=False),
                    gate.nonlinearity)

        # Add in all parameters from gates
        (self.W_in_to_updategate, self.W_hid_to_updategate, self.b_updategate, self.z_updategate,
         self.nonlinearity_updategate) = add_gate_params(updategate,
                                                         'updategate')
        (self.W_in_to_resetgate, self.W_hid_to_resetgate, self.b_resetgate,self.z_resetgate,
         self.nonlinearity_resetgate) = add_gate_params(resetgate, 'resetgate')

        (self.W_in_to_hidden_update, self.W_hid_to_hidden_update,
         self.b_hidden_update, self.z_hidden_update,self.nonlinearity_hid) = add_gate_params(
    ...
    def get_output_for(self, inputs, **kwargs):
        ...
        zr = self.z_resetgate
        zu = self.z_updategate
        zh = self.z_hidden_update
        ...
        def step(input_n, hid_previous, *args):
            ...
            for i in range(len(self.nonlinearity_resetgate)):
                resetgate = zr[i]*self.nonlinearity_resetgate[i](resetgate)
                updategate = zu[i]*self.nonlinearity_updategate[i](updategate)
            ...
            for i in range(len(self.nonlinearity_hid)):
                hidden_update = zh[i]*self.nonlinearity_hid[i](hidden_update)
        ...
        non_seqs = [W_hid_stacked,zr,zu,zh]
        # When we aren't precomputing the input outside of scan, we need to
        # provide the input weights and biases to the step function
        if not self.precompute_input:
            non_seqs += [W_in_stacked, b_stacked,zr,zu,zh]
